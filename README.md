...
- **`A373813_cover_costs.txt`** – simple two-column format: `N` and the optimal number of lines (easy for plotting).
- **`A373813_cover_stats.txt`** – detailed statistics for each N (time, search nodes, bounds, etc.). Generated by `primecover1024.cpp`.
- **`A373813_cover_lines.txt`** – the full list of line coordinates (in `(x, y)` format) for each optimal cover. Generated by `primecover1024_line_coordinates.cpp`.
