...
|------|------|-------------|
| `W` | Witness hit | The new point (N, p_N) already lies on a line in the cover inherited from step N−1, so f(N) = f(N−1) is certified in constant time with no search. |
| `R` | Root closed | The lower and upper bounds converged at the root node, certifying optimality without any branching or DFS. |
...
| `lb_cov` | The cover-inequality lower bound: the size of the largest subset S of the N active points such that no heavy line covers more than 2 members of S, yielding ⌈\|S\|/2⌉ as a valid lower bound. |
...
| `gap` | The optimality gap at certification: ub0 − lb. A gap of zero means the initial incumbent was already proven optimal without any search. |
...
| `strong_branch` | Total DFS nodes at which strong branching was invoked: up to 4 candidate lines are scored by simulating the include branch to select the variable that best prunes both child subtrees, triggered when the node gap is ≤ 1 and at least 64 points remain uncovered. |
| `depth` | Maximum DFS recursion depth reached across all parallel worker threads. |
| `forced` | Total heavy lines unconditionally forced into the solution by the Exclusive Dependency Rule across all DFS nodes and all threads — a line is forced when it is the sole productive heavy line available to at least 3 of its active points. |
