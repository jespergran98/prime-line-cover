/*
 * Experiment log
 * 1. Investigated the hard-tail search hot path with a sandbox memo probe, weaker warm-start dual schedules, deeper strong branching, and a 2x larger `113+` frontier split, all aimed at reducing the expensive exact cases around `N=814`.
 *    The measurements ruled out the obvious ideas: exact DFS-state memoization produced zero useful prunes at `N=814`; weakening the dual or removing warm-node polish either drove `N=780` into the 100s watchdog or worsened `N=814` from `38.998s / 1,588,502 nodes` to `42.074s / 2,032,503 nodes`; deeper strong branching also hit the 100s watchdog at `N=780`; and doubling the `113+` frontier split helped `N=808/812/814` (`25.276 -> 22.453s`, `18.347 -> 14.291s`, `38.998 -> 37.964s`) but made `N=813` explode from `25.115s / 1,574,852 nodes` to `42.228s / 2,774,387 nodes`.
 *    The simplest explanation is that this solver is extremely sensitive to dual/slack quality and branch order, so "cheaper bound" or "more split" changes mostly reshuffle work into larger trees instead of removing it; future attempts should avoid weakening coordinate polish or broadening strong branching without a new way to preserve the same decisions, and should focus instead on making the Lagrangian data path cheaper at identical decisions (for example incremental compact-line maintenance or faster slack reuse).
 * 2. Probed the `N=814` bottleneck by rewriting the compact productive-line rebuild path in sandbox copies and by testing tiny search-order/code-layout nudges (including a witness residual-line tie-break change and even removing an apparently dead `lagrangian_upper_bound(task, ctx)` wrapper) to see whether cheaper bound setup or a better warm path would collapse the hard tail.
 *    The probe confirmed the rebuild path is expensive in aggregate (`build_compact` about `29-32s` and productive-metadata rebuild about `23-24s` summed over the `N=814` search), but neither rebuild rewrite produced a repeatable improvement, and the strongest-looking result from the dead-wrapper removal was a false positive: one run dropped to `136.675s / 5,329,356 nodes` because `N=813` closed in witness mode, while the required confirmation pair regressed to `161.278s / 6,844,779 nodes` and `161.917s / 6,864,313 nodes` versus the baseline `159.299-159.439s`.
 *    The simplest reason is that these edits changed warm-start/search trajectory and codegen sensitivity far more than they removed actual work, so the occasional win came from a different incumbent path rather than a cheaper exact solve (this same behavior of random drops have also been spotted in the baseline without any edits); future attempts should avoid trusting single-run outliers or layout-only changes, require paired tail validation, and focus on deterministic slack/compact-state reuse that preserves the same branch and dual decisions.
 */
...
namespace config {
    ...
    constexpr double kPerNTimeLimitSeconds = 100.0;
} // namespace config
...
private:
    ...
    void apply_forced_line_reduction(Task& task, WorkerContext& ctx, SelectLineFn&& select_line_fn) const;
...
long double ExactGainSolver::lagrangian_upper_bound(const Task& task, WorkerContext& ctx) const {
...
//
template <typename SelectLineFn>
void ExactGainSolver::apply_forced_line_reduction(Task& task, WorkerContext& ctx, SelectLineFn&& select_line_fn) const {
...
void ExactGainSolver::on_productive_line_removed(Task& task, int id) const {
    for (const CompactPointIndex point_cp : line_points(id)) {
        const int p = static_cast<int>(point_cp);
        if (p >= cur_n_) break;
        if (!task.active.test(p)) continue;
...
void ExactGainSolver::on_productive_line_reinserted(Task& task, int id) const {
    for (const CompactPointIndex point_cp : line_points(id)) {
        const int p = static_cast<int>(point_cp);
        if (p >= cur_n_) break;
        if (!task.active.test(p)) continue;
...
void ExactGainSolver::remove_productive_impl(Task& task, int id, std::vector<int>* removed_log) const {
...
void ExactGainSolver::reinsert_productive(Task& task, int id) const {
