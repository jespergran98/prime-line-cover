/*
 * Experiment log
 * 2. Tried reducing the coordinate-polish phase inside lagrangian_upper_bound so the late D-tail would spend less time polishing duals on near-threshold nodes. Five full warm-start sweeps to N=805 were tested with coarse polish gates: disabling it entirely, limiting it to fresh-dual calls, restricting it to active-point thresholds of 224 and 256, and tightening the trigger window from 0.5 to 0.25; the aggressive gates timed out early (at N=730, N=765, and N=784), while the mild window reduction completed but was slower than baseline at the hard tail. The simplest reason is that polish is expensive but still pruning-critical, so broad gates save per-bound work while expanding the search tree even more. Future attempts should avoid global polish cutbacks unless they can identify, with state-specific evidence, nodes where polish contributes no meaningful pruning.
 * 1. Tried replacing the blocked-productive set's sorted insert/erase maintenance with O(1) swap/push updates, sorting only when frontier state keys were materialized, to cut per-node bookkeeping in the DFS hot path. Across two full post-change sweeps to N=805, the updated solver was not consistently faster than baseline and the tail stayed effectively flat, with average total time drifting slightly upward overall. The simplest explanation is that frontier-key sorting and normal run-to-run variance outweighed the saved maintenance work because the dominant tail cost is still the repeated bound/search effort, not blocked-set bookkeeping alone. Future attempts should avoid treating blocked-set ordering as the primary tail bottleneck unless they also reduce the amount of branch-and-bound work itself.
 */
