 ...
 *   20. A residual cover-inequality lower bound inside DFS/frontier expansion
 *       was supposed to close the hard gap=1 tail earlier by turning the
 *       existing root-only structural cut into a branch-level prune. The fast
 *       metadata-driven version broke exactness immediately (for example N=730
 *       rose from the 97-line baseline to 98 and N=800 rose from 111 to 112),
 *       and two exact recomputed variants still slowed the full 800 sweep to
 *       129.797s and 125.494s versus the 112.130s / 113.619s baseline pair
 *       while making N=789/N=795/N=800 worse. The simplest explanation is
 *       that this residual cut is either unsafe when it leans on maintained
 *       heuristic metadata or too expensive to recompute often enough to pay
 *       for the extra pruning, so future attempts should avoid branch-level
 *       cover-inequality pruning unless the residual state is proven exact and
 *       the trigger is narrow enough to save more bound work than it adds.
 *   21. Running a full Lagrangian exclude-child evaluation during gap-1
 *       strong branching was supposed to prune more dead exclude branches by
 *       scoring both strong-branch children with the same bound quality. Three
 *       800 sweeps all lost where it mattered: the full-shortlist version
 *       finished at 113.730s with N=789/N=795/N=800 at 13.744s/20.808s/31.130s,
 *       the analytically-pruned-only version still slowed that tail to
 *       13.792s/20.605s/30.933s despite a 111.344s total, and the
 *       shortlist-leader-only follow-up regressed again to 113.517s with
 *       N=789 at 14.496s and N=795 at 21.404s, versus the 109.818s / 113.734s
 *       baseline pair. The simplest explanation is that these extra
 *       exclude-child Lagrangian solves mostly revisit the same gap-1 states
 *       without changing the search tree enough to repay their cost, so future
 *       attempts should avoid another full strong-branch exclude solve unless
 *       the work can be reused across candidates or triggered by a much tighter
 *       signal.
 */
...
struct ExactSolveResult {
    ...
    int              lb_lp = 0;      // root lower bound from fractional_cover_lp_lb()
...
private:
        ...
        int* greedy_gain_out) const
    {
        ...
        auto& work_line_slack = scratch.work_line_slack;
        ...
        auto eval_dense_fast = [&](const std::vector<DualValue>& yy,
            ...
            for (std::size_t li = 0; li < compact_line_ids.size(); ++li) {
                ...
                const int rem = static_cast<int>(
                    task.line_cover[static_cast<std::size_t>(id)]);
                const DualValue rc = static_cast<DualValue>(rem - 2) - sum_y;
                if (rc > 1e-18) {
                    value += rc;
                    if (subgradient != nullptr) {
                        for (std::size_t pos = compact_offsets[li];
        ...
        if (kCoordinatePolishMaxSweeps > 0) {
            for (std::size_t li = 0; li < compact_line_ids.size(); ++li) {
                ...
                work_line_slack[static_cast<std::size_t>(id)] = slack;
            }
            ...
            auto slack_objective = [&]() -> DualValue {
                ...
                for (int id : compact_line_ids)
                    if (work_line_slack[static_cast<std::size_t>(id)] > 1e-18)
                        value += work_line_slack[static_cast<std::size_t>(id)];
                return value;
            };
            ...
            for (int sweep = 0; sweep < kCoordinatePolishMaxSweeps; ++sweep) {
                ...
                for (int p : active_points) {
                    ...
                    for (int id : incidence_[static_cast<std::size_t>(p)]) {
                        ...
                        const DualValue adjusted =
                            work_line_slack[static_cast<std::size_t>(id)] + old;
                    ...
                    for (int id : incidence_[static_cast<std::size_t>(p)]) {
                        ...
                        const int rem = static_cast<int>(task.line_cover[static_cast<std::size_t>(id)]);
                        if (rem < 3) continue;
                        work_line_slack[static_cast<std::size_t>(id)] -= delta;
                    }
                }
...
int main(int argc, char** argv) {
    ...
    for (int n = kStartN; n <= requested_n; ++n) {
        ...
        // current-N root cuts (cover inequality / LP) can strengthen it.
