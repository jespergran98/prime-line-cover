...
constexpr int    kExecutionLimit       = 270;   // hard cap and default run-to N
// Maximum wall-clock seconds allowed per N. Set to 0.0 to disable (no limit). 60=1 min
constexpr double kPerNTimeLimitSeconds = 35;
...
// with no disjointness assumption.  It branches by partitioning solutions on
// the first gain-positive line through a pivot point.
// ---------------------------------------------------------------------------
...
private:
    ...
    [[nodiscard]] int choose_branch_point(
        const Task& task, std::vector<int>& candidates) const
    {
        int best_point = -1;
        long double best_avg = -1.0L;
        int best_max_gain = -1;
        int best_options = std::numeric_limits<int>::max();
        ...
        task.active.for_each_set_bit(words_, [&](int p) {
            ...
            const long double avg =
                static_cast<long double>(sum_gain) /
                static_cast<long double>(local_candidates.size());
            const bool better =
                (avg > best_avg + 1e-18L) ||
                (std::abs(avg - best_avg) <= 1e-18L && max_gain > best_max_gain) ||
                (std::abs(avg - best_avg) <= 1e-18L && max_gain == best_max_gain &&
                 static_cast<int>(local_candidates.size()) < best_options) ||
                (std::abs(avg - best_avg) <= 1e-18L && max_gain == best_max_gain &&
                 static_cast<int>(local_candidates.size()) == best_options && p < best_point);
            if (better) {
                best_point = p;
                best_avg = avg;
                best_max_gain = max_gain;
                best_options = static_cast<int>(local_candidates.size());
                best_lines = std::move(local_candidates);
            }
        });

        candidates = std::move(best_lines);
        return best_point;
    }
    ...
    [[nodiscard]] long double lagrangian_upper_bound(const Task& task) const {
        ...
        if (static_cast<long double>(task.current_gain) + best <=
            static_cast<long double>(current_best_gain()) + 1e-9L)
        ...
        for (int iter = 0; iter < 12; ++iter) {
            ...
            const long double incumbent_gap =
                std::max(0, current_best_gain() - task.current_gain);
    ...
    void dfs(Task& task) {
        ...
        if (ub <= static_cast<long double>(current_best_gain()) + 1e-9L) return;

        maybe_submit_greedy(task);

        std::vector<int> candidates;
        const int point = choose_branch_point(task, candidates);
        if (point < 0) {
            submit_candidate(task.current_gain, task.current_choice);
            return;
        }

        // Partitioning is by the first chosen line through `point`. If two
        // candidates cover the same active points, choosing the earlier one
        // yields the same child state and dominates the later one.
        std::vector<BitMask1024> seen_children;
        seen_children.reserve(candidates.size());

        {
            UndoLog undo;
            for (int id : ordered_point_lines_[static_cast<std::size_t>(point)])
                disable_line(task, undo, id);
            dfs(task);
            undo_to(task, undo);
        }

        for (int chosen_line : candidates) {
            const BitMask1024 rem = task.active & lines_[static_cast<std::size_t>(chosen_line)].mask;
            if (is_duplicate_active_coverage(rem, seen_children)) continue;
            seen_children.push_back(rem);

            UndoLog undo;
            for (int id : ordered_point_lines_[static_cast<std::size_t>(point)]) {
                if (id == chosen_line) break;
                disable_line(task, undo, id);
            }

            const int delta = positive_gain(task, chosen_line);
            if (delta <= 0) {
                undo_to(task, undo);
                continue;
            }

            select_line(task, undo, chosen_line);
            task.current_gain += delta;
            task.current_choice.push_back(chosen_line);
            dfs(task);
    ...
    void expand_task(const Task& task, std::vector<Task>& children) const {
        ...
        for (int chosen_line : candidates) {
            ...
            for (int id : ordered_point_lines_[static_cast<std::size_t>(point)]) {
                if (id == chosen_line) break;
                if (child.available[static_cast<std::size_t>(id)])
                    child.available[static_cast<std::size_t>(id)] = 0;
            }
            const int delta = positive_gain(child, chosen_line);
            if (delta <= 0) continue;
            child.current_gain += delta;
            child.current_choice.push_back(chosen_line);
            select_line_without_undo(child, chosen_line);
    ...
    [[nodiscard]] std::vector<Task> build_frontier(Task root) {
        ...
        while (frontier.size() < target) {
            ...
            const long double ub = lagrangian_upper_bound(task);
            if (ub <= static_cast<long double>(current_best_gain()) + 1e-9L) {
