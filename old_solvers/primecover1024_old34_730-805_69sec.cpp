 ...
 *  
 * Experiment Log:
 *   4. We tested moving the warm-start coordinate polish ahead of the Lagrangian subgradient loop on dual-ready nodes so late-tail states could try a cheap prune before paying for a full dual solve, with broader and tighter gates based on incumbent cost and prune-window distance.
 *      In sweeps to N=805, the broad version cut nodes and the hardest tail cases (run3 78.563s / 1,166,707 nodes, N=805 26.812s; run4 79.647s / 1,169,130 nodes, N=805 26.905s) but still missed the full-sweep consistency bar versus the baseline's 78.593s-79.005s total and 27.056s-27.856s at N=805; the tighter variants also stayed slower overall at 79.350s, 79.401s, and 79.286s.
 *      The simplest reason is that the extra pre-polish work helps the hardest tail nodes but lands on too many medium nodes, so the saved subgradient iterations and node cuts do not repay the added per-node overhead across the whole warm-start sweep.
 *      Future attempts should avoid another always-on or lightly gated warm-start dual-tightening pass unless it is driven by a sharper predictor than incumbent cost/window alone and proves a total-time win on the full N=730..805 sweep.
 *   3. We tested stronger branching-score lookahead in the DFS selector, with three variants that widened the trigger to gap<=2, raised the lookahead depth cap from 6 to 10, and then restricted the extra scoring to the late tail with current_best_cost() >= 109.
 *      In full warm-start sweeps to N=795, none produced a clear win: the broad depth-10 version cut N=795 to 10.228s but raised total time to 35.84s, the late-tail depth-10 gate still took 35.12s with N=795 at 10.483s, and the late-tail depth-6 gate finished at 35.42s with N=795 at 10.282s versus the baseline's 34.56s-35.06s total and 10.329s-10.380s at N=795.
 *      The simplest reason is that the extra probe solves improved branch ordering too rarely, while their scoring cost landed on many medium-hard nodes before the worst tail could repay it.
 *      Future attempts should avoid broader strong-branching triggers unless they are driven by a much sharper predictor of imminent subtree explosion in the warm-start tail.
 *   2. We tested a tighter compact-builder pipeline for the Lagrangian bound that tried to remove setup passes by folding compact_rem2 into the productive-line scan, assigning/remapping compact point indices earlier, and fusing the arity-classification pass into the remap loop.
 *      In full N=730..805 sweeps, every variant failed to beat the kept baseline: the aggressive single-pass reorder regressed to 82.38s / 1,356,054 nodes with N=805 at 28.544s, the order-restored version still took 81.02s / 1,298,645 nodes with N=805 at 27.600s, and the fused-pass variant landed near-baseline but slower at 77.90s / 1,293,970 nodes with N=805 at 27.152s versus the baseline's 77.05s-77.21s total and 26.799s-26.853s at N=805.
 *      The simplest reason is that the saved compaction work was too small, and the more aggressive rewrites disturbed the hot dense-array locality and search behavior enough to give the time back in the tail.
 *      Future attempts should avoid micro-optimizing the compact-builder pipeline unless they preserve the original active-point ordering and show a clear wall-clock win on the full warm-start sweep.
...
namespace config {
    ...
    constexpr int    kExecutionLimit       = 805;   // hard cap and default run-to N
...
private:
    ...
    void apply_forced_line_reduction(Task& task, WorkerContext& ctx, bool record_undo) const {
        // Profiling showed the undo-capable DFS path spending more time
        // rescanning for 4-unique-point kernel lines than it recovered from the
        // very rare forced picks in the late tail. Keep the reduction in the
        // root/frontier setup, but skip that hot DFS-only scan.
        if (record_undo) return;
