 ...
 * Experiment Log:
 *
 *   1.  Lagrangian‑bound speedups (warm‑start cutoff, stall stop, compact‑builder rewrite)
 *       - What was tried:
 *           • Early warm‑start cutoff to stop Lagrangian refinement early.
 *           • Warm‑start stall stop to freeze dual variables when progress stagnates.
 *           • Compact‑builder rewrite to remove redundant setup passes.
 *       - Results:
 *           • First two reduced bound work per call but enlarged the search tree
 *             significantly, increasing nodes and runtime on hard cases (N=786,789,795,800).
 *           • Compact‑builder rewrite crashed with heap corruption before any benefit.
 *       - Why it didn't help:
 *           The solver is more sensitive to bound quality and compact‑incidence invariants
 *           than to raw overhead. Weakening the refinement path or breaking invariants
 *           harms pruning more than the speedup helps.
 *       - Future attempts should avoid:
 *           • Reducing Lagrangian iteration count without preserving dual quality.
 *           • Refactoring the compact builder without much tighter invariant checks.
 *
 *   2.  Cheap total‑gain upper bound before Lagrangian call
 *       - What was tried:
 *           Added a fast bound: Σ (line_cover[id] - 2) over productive lines,
 *           called before lagrangian_upper_bound() in dfs(), run_task(), and build_frontier().
 *       - Results:
 *           Runtime increased from 64 s to 66 s (+3%) over N=730–800.
 *           No reduction in node count or bound evaluations.
 *       - Why it didn't help:
 *           The cheap bound is almost never tighter than the existing node_gain_upper_bound
 *           (which already uses Lagrangian values). The extra sum over productive_ids added
 *           non‑trivial overhead per node without eliminating any expensive bound calls.
 *       - Future attempts should avoid:
 *           Adding per‑node checks that do not substantially prune the tree.
 *           Instead, focus on reducing the constant factor of lagrangian_upper_bound()
 *           itself (e.g., early termination when dual stops improving, or using a
 *           cheaper heuristic bound like greedy as a first filter).
 */
...
private:
    ...
    struct LagrangianScratch {
        ...
        std::vector<int>         compact_point_aps;
    ...
    void apply_forced_line_reduction(Task& task, WorkerContext& ctx, bool record_undo) const {
        ...
        while (true) {
            ...
            if (record_undo && as_int(task.current_choice.size()) <= 38) { // anything below 38 causes incorrect line counts at the 786th prime.
                const ForcedChoice exact = exact_forced_choice(task, ctx);
                best_line = exact.best_line; best_unique = exact.best_unique; best_gain = exact.best_gain;
            }
    ...
    [[nodiscard]] int greedy_completion_compact(
        ...
        const std::vector<std::size_t>& compact_offsets, const std::vector<int>& compact_points, bool keep_lines) const
    {
        BitMask1024 temp_active = task.active;
        const int max_gain = std::max(0, task.active_count - 2);
        std::fill_n(ctx.greedy_bucket_head.begin(), max_gain + 1, -1);
        ...
        for (int bucket = max_gain; bucket > 0; --bucket) {
            for (int li = ctx.greedy_bucket_head[as_size(bucket)]; li >= 0; li = ctx.greedy_bucket_next[as_size(li)]) {
                ...
                for (std::size_t pos = compact_offsets[line_index]; pos < compact_offsets[line_index + 1]; ++pos) {
                    if (temp_active.test(compact_points[pos])) ++current_gain;
                }
                if (current_gain <= 0) continue;
                ...
                for (std::size_t pos = compact_offsets[line_index]; pos < compact_offsets[line_index + 1]; ++pos) {
                    if (temp_active.test(compact_points[pos])) temp_active.reset(compact_points[pos]);
                }
            }
        }
        return gain;
    ...
    [[nodiscard]] long double lagrangian_upper_bound(const Task& task, WorkerContext& ctx, int max_iters, int* greedy_gain_out) const {
        ...
        auto eval_dense_fast = [&](const std::vector<DualValue>& yy, std::vector<DualValue>* subgradient) -> DualValue {
            ...
            for (int ai = 0; ai < active_point_count; ++ai) {
                value += yy[as_size(ai)];
                if (subgradient) (*subgradient)[as_size(ai)] = 1.0;
            }
            for (std::size_t li = 0; li < compact_line_ids.size(); ++li) {
                DualValue sum_y = 0.0;
                for (std::size_t pos = compact_offsets[li]; pos < compact_offsets[li + 1]; ++pos)
                    sum_y += yy[as_size(compact_point_aps[pos])];
                const DualValue rc = static_cast<DualValue>(compact_rem2[li]) - sum_y;
                if (rc > 1e-18) {
                    value += rc;
                    if (subgradient) {
                        for (std::size_t pos = compact_offsets[li]; pos < compact_offsets[li + 1]; ++pos)
                            (*subgradient)[as_size(compact_point_aps[pos])] -= 1.0;
                    }
                }
            }
            return value;
        ...
        auto eval_dense_exact = [&](const std::vector<DualValue>& yy, bool store_slack) -> long double {
            ...
            for (int ai = 0; ai < active_point_count; ++ai)
                value += static_cast<long double>(yy[as_size(ai)]);
            for (std::size_t li = 0; li < compact_line_ids.size(); ++li) {
                const int id = compact_line_ids[li];
                long double sum_y = 0.0L;
                for (std::size_t pos = compact_offsets[li]; pos < compact_offsets[li + 1]; ++pos)
                    sum_y += static_cast<long double>(yy[as_size(compact_point_aps[pos])]);
                const long double rc = static_cast<long double>(compact_rem2[li]) - sum_y;
        ...
        if (kCoordinatePolishMaxSweeps > 0) {
            ...
            for (std::size_t li = 0; li < compact_line_ids.size(); ++li) {
                ...
                for (std::size_t pos = compact_offsets[li]; pos < compact_offsets[li + 1]; ++pos)
                    slack -= active_best_y[as_size(compact_point_aps[pos])];
                wls_c[li] = slack;
            }
    ...
    void dfs(Task& task, WorkerContext& ctx) {
        ...
        if (seen_search_state(task, ctx)) { rollback(); return; }
        
        int greedy_gain = 0;
        const long double ub = lagrangian_upper_bound(task, ctx, &greedy_gain);
        capture_root_cost_lb(ub);
        
        if (root_interval_closed() || gain_upper_bound_cannot_beat_best_cost(ub)) { rollback(); return; }
...
int main(int argc, char** argv) {
    ...
    for (int n = kStartN; n <= requested_n; ++n) {
        ...
        std::println("N={} prime={} active={} mode={} lb_floor={} lb_cov={} lb_lp={}"
                     " ub0={} lb={} gap={} ub_raw={} lb_raw={} gap_raw={}"
                     " prod={} pinc={} lines={} time={} nodes={}",
            n,
            primes[as_size(n - 1)],
            active_count,
            mode,
