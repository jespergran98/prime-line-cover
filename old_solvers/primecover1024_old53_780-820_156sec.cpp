/*
 * Experiment log
 * 1. I tested stronger future-witness construction, deeper frontier splitting, and a no-rebuild forced-line path against the 780..820 sweep to attack the dominant tail spikes at N=808/812/813/814/818. Residual-pair beam search never moved the expensive same-answer misses, and a best 1-swap heavy-line polish only improved the N=808 witness from next miss 810 to 812, so the 812/813/818 solves are not mainly blocked by leftover pairing or a trivial local heavy-line repair.
 *    More aggressive frontier splitting (512x at 113+, 128x at 110+) made the full sweep much worse at 211.883s and blew up N=818 to 48.083s / 2,623,814 nodes, while removing the forced-metadata rebuilds made the run fast but wrong by inflating answers to 114 at N=813 and 115 at N=818. Future attempts should avoid pair-only witness work, avoid frontier over-splitting, and not trust incremental unique-line metadata without a sound dirty-point scheme; the most promising remaining direction is a correctness-preserving heavy-line re-optimization or witness-aware incumbent generator.
 */
...
private:
    ...
    // any point of U). The rule applies when |U| ≥ 3. The proof shows Δ ≤ 0
    // for |U| = 3 (Δ = 0) and Δ < 0 for |U| ≥ 4, so forcing L never worsens
    // the solution.
    //
    ...
    //   cover the d non‑U points using at most ⌈d/2⌉ ≤ ⌈k/2⌉ new pairing
    //   lines.  The change in total cost is
    //        Δ = –k + 1 + ⌈k/2⌉.
    //   For k = 2, Δ = 0; for k = 3, Δ = 0; for k ≥ 4, Δ ≤ –k/2 + 1.5 < 0.
    //   Because |U| ≥ 3 forces k ≥ ⌈|U|/2⌉ ≥ 2, we have Δ ≤ 0.  Hence there
    ...
    //
    // DFS-safe variant: rebuilds productive metadata from scratch before each
    // outer call so that staleness from parent branching decisions cannot cause
    // false forced selections.  Uses select_line (undo-aware) so that all
    // changes are automatically rolled back when the node backtracks via
    // undo_to().  Called immediately after mark_undo() at the top of dfs().
    // ----------------------------------------------------------------------
    void apply_dfs_forced_line_reduction(Task& task, WorkerContext& ctx) {
        auto& touched = ctx.forced_touched_lines;

        while (true) {
            // Rebuild metadata from scratch for all active points so that
            // staleness from parent branching decisions cannot cause false
            // forced selections.  Uses select_line (undo-aware) so that all
            // changes are automatically rolled back when the node backtracks.
            std::fill(task.productive_degree.begin(),
                      task.productive_degree.begin() + cur_n_, 0);
            std::fill(task.sole_productive_line.begin(),
                      task.sole_productive_line.begin() + cur_n_, kNoProductiveLine);
            for (int id : task.productive_ids) {
                for (CompactPointIndex cp : line_points(id)) {
                    const int p = static_cast<int>(cp);
                    if (p >= cur_n_) break;
                    if (!task.active.test(p)) continue;
                    int& deg = task.productive_degree[as_size(p)];
                    if (deg == 0) task.sole_productive_line[as_size(p)] = id;
                    else          task.sole_productive_line[as_size(p)] = kManyProductiveLines;
                    ++deg;
                }
            }
            ...
            task.active.for_each_set_bit(words_, [&](int p) {
                ...
                if (unique_line >= 0) {
                    if (touched.empty() || touched.back() != unique_line)
    ...
    // apply_root_forced_line_reduction uses select_line_without_undo and is
    // called only at the root / frontier where metadata is already consistent.
    // Inside dfs(), use apply_dfs_forced_line_reduction instead.
    // ----------------------------------------------------------------------
    ...
    void apply_root_forced_line_reduction(Task& task, WorkerContext& ctx) const {
        ...
        while (true) {
            // Rebuild productive_degree / sole_productive_line from scratch so
            // that stale incremental state cannot cause incorrect forced selections.
            std::fill(task.productive_degree.begin(),
                      task.productive_degree.begin() + cur_n_, 0);
            std::fill(task.sole_productive_line.begin(),
                      task.sole_productive_line.begin() + cur_n_, kNoProductiveLine);
            ...
            for (int id : touched) {
                ...
                const bool better = (unique_points > best_unique) ||
                                    (unique_points == best_unique && gain > best_gain) ||
                                    (unique_points == best_unique && gain == best_gain &&
                                     (best_line < 0 || id < best_line));
                if (better) {
