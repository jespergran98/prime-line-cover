/*
 * Experiment log
 * 1. Investigated whether the 113/114 tail was dominated by redundant DFS states or by overspending inside the Lagrangian bound. A bounded transposition cache produced effectively zero useful prunes on the hard cases, while profiling on N=800/805/808/812/813/814 showed the bound split is roughly 58-60% fast evaluation, 16-18% compact rebuild, 13-14% coordinate polish, and 7-8% exact confirmation, so the hot spot is the numeric bound kernel rather than repeated search states.
 *    Dual precision reduction (`double` -> `float`) and more aggressive tail strong branching both preserved answers but did not help: the float build regressed to `total time=159.303s` with `N=814 time=38.604s nodes=1899571`, and the stronger-branching build finished at `total time=149.286s` with `N=814` essentially unchanged at `36.199s` and `1588506` nodes. Coordinate polish is expensive but necessary: disabling it made even `N=780` explode past 38s, and capping cold-start polish to one sweep only nudged `N=814` down to `35.681s` while leaving `total time=148.101s`, so future attempts should avoid weakening the dual bound or spending effort on DFS deduplication/branch-order tuning before improving the fast evaluation kernel itself or finding a materially stronger exact lower bound for the 113/114 transition.
 * 2. Tried future-aware warm-start selection to make plateau witnesses survive farther into the sweep: one variant broke ties between equal-cost incumbents by preferring the cover whose existing witness reached farther ahead, a second widened that rule to `lb_floor + 1` covers, and a third biased branch ordering toward heavy lines that hit more future points. Probe runs showed the key fact is real but awkward: on the slow path the `N=812` 113-line cover carried witness bits `00101011`, so the expensive `N=813` solve is caused by landing on the wrong optimal heavy-line set rather than by the residual witness builder, and one lucky plateau-only run did cut `total time` to `127.571s` by flipping `N=813` to `W`.
 *    The problem is that the plateau search stops as soon as the first floor-cost cover is found, so tie-breaking after `submit_candidate` is too race-sensitive to be reliable: repeated runs of the same patch bounced back to `N=813 mode=D` with `total time=149.002s`, while the broader tie-break and future-biased branching regressed further to `151.669s` and `150.618s`. Future attempts should avoid witness-polishing schemes that do not also change when plateau search terminates; the useful rediscovery here is that future performance is extremely sensitive to which exact 113/114 cover is exported, but exploiting that safely will need a dedicated secondary-objective search or a deterministic post-optimal repair rather than shallow incumbent tie-breaks.
 * 3. Tried a bounded secondary plateau-repair search after hard solves, using a sandbox-only harvest mode that collected a few same-cost covers and rescored them by future witness reach before exporting the warm start. A broad trigger was immediately harmful (`N=780 time=3.942742s` vs the ~`2.26-2.31s` baseline), tightening the trigger to the 113/114 tail still regressed to `total time=154.322s`, and a final first-plateau-only variant remained slower at `148.575s` with `N=813 time=24.050s`, `N=814 time=36.124s`, and `N=818 time=11.249s`.
 *    The useful finding is that a small second search can change witness behavior locally, but under practical node budgets it mostly adds duplicate DFS/Lagrangian work without finding the rare future-good cover quickly enough to offset the extra pass. The simplest reason it failed is that “best short-prefix witness among the first few alternate covers” is not a strong enough proxy for the truly valuable plateau cover, so future attempts should avoid bounded same-N alternative-cover harvesting unless they can target a specific future feasibility goal or use a much sharper secondary score than immediate witness prefix.
 * 4. Tried two classes of changes aimed at the measured Lagrangian hot path: a dual-kernel rewrite that removed compact point renumbering and evaluated directly on original point ids, plus scheduler variants that changed only parallel tail decomposition (worker caps of 8/12 and heavier frontier targets of 3072/4096 tasks). The representation rewrite preserved answers but regressed the anchor case to `N=814 time=36.620s`; a probe run on `N=808/812/813/814` confirmed the true work split is stable at roughly `build=16.9-17.2%`, `fast eval=60.5-60.9%`, `exact=8.2-8.4%`, `polish prep=4.6%`, and `polish=9.0-9.5%`, with about `259-269` active dual points, `700-751` productive lines, and `2334-2513` active incidences per bound call.
 *    The scheduler tweaks showed the tail is highly path-sensitive rather than purely throughput-limited: `8` workers improved `N=812` to `13.731s` but timed out at `N=813 > 40s`, `12` workers improved `N=808` to `22.746s` but regressed `N=814` to `37.960s`, a `3072`-task frontier cut `N=814` to `31.813s` and `N=812` to `11.181s` but pushed `N=813` to `32.156s` and `N=818` to `24.543s`, for a full-run `total time=155.411s`. The simplest reason none of these helped overall is that compact dual indices are buying real cache locality in the 60% fast-eval kernel, while scheduler-only changes perturb which optimal cover gets exported enough to move cost between `813/814/818` instead of reducing it; future attempts should avoid non-compact dual rewrites and tail scheduling tweaks unless they are validated against the whole `N=800..820` block rather than a single favorite hard case.
 * 5. Tried two follow-ups aimed at the same measured bottleneck: a cheap root-floor early return, and a compact incremental line-sum Lagrangian kernel motivated by a probe on `N=814` showing only `70.692 / 795.572` productive lines (about `8.9%`) have positive reduced cost per subgradient step while `120.278 / 278.828` active dual coordinates (about `43.1%`) still move each iteration. The incremental kernel preserved answers in sandbox probes but the bookkeeping swamped the saved reduced-cost checks, with the full sweep already badly behind by `N=800 time=15.086s`, `N=805 time=15.492s`, and `N=808 time=23.544s`, so future attempts should avoid stateful per-node dual-maintenance schemes unless the point-to-line incidence graph can be reused across many iterations instead of rebuilt for every bound call.
 *    The root-floor fast-path was the only consistently good sub-change: in the controlled seed run it cut trivial root-closure cases such as `N=801/806` from about `0.06s` to about `0.009s`, but it did not touch the real tail work (`N=813 time=13.396s`, `N=821 time=20.298s`) and only moved the full run to `total time=133.628s` from the `134.762s` baseline while remaining slower than the best baseline path. The simplest reason the round failed is that the expensive cases are still dominated by genuine `mode=D` search plus the compact dual kernel, so future attempts should avoid optimizations that only accelerate `mode=R/W` closures unless they also strengthen the lower bound or deterministically prevent the `N=813` witness miss.
 * 6. Tried three sandbox-only rewrites of the measured dual hot path. Two coordinate-descent-first variants were supposed to replace part of the `~60%` fast-evaluation work on the warm 113/114 tail, but the aggressive version timed out at `N=808` (`[stopped] ... 45.016s`) and the milder version was already behind the full baseline by `N=814`, so future attempts should avoid primary-dual rewrites that trade subgradient pruning strength for nicer per-iteration local optimization.
 *    A separate compact-build rewrite removed the productive-union/remap pass inside `build_compact_productive_lines()` by building the active dual core directly from `productive_degree`, which did preserve counts and locally helped several hard solves (`N=808 14.290s -> 13.893s`, `N=812 10.836s -> 10.583s`, `N=818 11.786s -> 11.578s`). The useful failure data is that this modest kernel win still flipped `N=813` from the good baseline `mode=W` into `mode=D time=13.334s`, sending the full sweep to `total time=133.670s`; the simplest reason is that even small timing changes can perturb which equal-cost cover gets exported, so future attempts should avoid low-level tail speedups unless they also stabilize post-optimal cover selection across the whole `N=800..825` block.
 * 7. Tried several witness-propagation rewrites aimed at the expensive same-cost plateau misses: rebuilding the witness after every `mode=W` step, reusing a bank of older witness covers, and reusing older exact heavy-line sets by rebuilding fresh current-`N` witnesses from them. Rebuilding on `W` was directionally useful and consistently flipped `N=825` from an `~8.4s mode=D` solve into `~0.007s mode=W`, but the full runs still landed at `total time=123.598s` and `122.496s`, not better than the validated `122.289s` baseline because `N=812/813/818` remained hard and the tail path just shifted around.
 *    The banked-cover variant exposed an important correctness trap: checking only the new point against a witness older than `N-1` is unsound, because it falsely claimed `N=821..823 lines=114`; rebuilding a fresh witness from historical heavy sets fixed that bug but regressed to `total time=132.963s`. The simplest reason this family failed is that stale residual pairings explain only a small slice of the plateau misses, while broader multi-step witness reuse still does not stabilize enough of the 113/114 transition to beat the best path, so future attempts should avoid older-witness reuse unless they construct a fresh current-`N` cover and verify the full line count rather than point membership alone.
 * 8. Tried three next-step rescue variants aimed at the observed `N=812 -> 813` witness cliff: a targeted future-feasibility repair search after exact solves, a one-step witness rebuild plus one extra cold re-solve only when a same-cost plateau still missed `N+1`, and a colder tail policy that dropped the warm start once the sweep reached the 113-line regime. The dedicated repair search was immediately harmful (`total time=166.704s` with `N=812 time=28.775s`, `N=813 time=25.460s`), while the one-step-plus-retry build produced a very strong lucky run (`total time=117.960s`, `N=813 mode=W`, `N=825 mode=W`) but failed confirmation when the retry path exploded at `N=812 time=56.051s` and tripped the per-N stop; the broad cold-tail solve was worse still, reaching `N=812 time=44.260s`, `N=813 time=29.519s`, and then timing out at `N=814`.
 *    The useful finding is that there really is a high-value alternate plateau cover hiding near the `812/813` transition, but any approach that pays for a second full same-`N` exact solve or throws away the incumbent structure can turn one hard plateau into two harder ones. The simplest reason this round failed is that witness-miss detection alone is too weak a trigger for another exact pass, so future attempts should avoid unconditional re-solves or cold-start policies unless they can cheaply predict that the first solve landed on the bad optimal cover rather than on a genuinely hard node.
 * 9. Tried three deterministic warm-start repair variants aimed at avoiding the expensive incumbent search rather than speeding up the dual kernel: a one-swap post-optimal heavy-line export repair scored by future witness bits, a sound current-`N` seed-feasibility check plus one-swap local repair before `solve_exact()`, and a one-line augmented-greedy seed heuristic around the incoming warm heavy set. All preserved line counts, but the best full sweep still landed at `total time=140.172s` while the other two finished at `141.118s` and `145.222s`, and the anchor cliffs stayed hard in the best build (`N=813 time=21.245s`, `N=814 time=22.202s`, `N=821 time=21.552s`), so none beat the validated baselines `136.682s` and `141.098s`.
 *    The useful finding is that the family is not empty: a post-solve swap at `N=808` improved the exported witness bits from `10100010000000000` to `11100010100101000`, and the sound seed-feasibility test skipped exact work at `N=816 residual=24 lines=114` and `N=823 residual=22 lines=115`, but the profitable `113/114/115` plateau cover was still not reachable through these tiny neighborhoods. The simplest reason this round failed is that the bad plateau incumbents are not one swap, one residual-feasibility check, or one added greedy line away from the cover we need, so future attempts should avoid more single-move warm-start repair heuristics unless they search a materially larger neighborhood or can prove the target cover is locally reachable.
 * 10. Tried four sandbox variants aimed at the observed incumbent-search bottleneck rather than the dual kernel: a post-floor plateau probe, a deterministic destroy-and-rebuild seed/LNS heuristic, selective in-search greedy completion, and a residual subproblem cover-inequality lower bound. The probe showed why the tail is stubborn: at `N=812` the first 113-line incumbent arrived after `140875` nodes and no second same-cost cover appeared in the next `250k` nodes, while at `N=813` the first 113-line incumbent arrived after `1586488` nodes with again no alternate same-cost cover in the next `250k`; both incumbents still missed `N+1` and differed from the incoming warm heavy set by `34-35` lines, and a separate structural probe confirmed the root heavy-line graph stays a single connected component through `N=800/805/808/812/813/814/818/821/825`.
 *    The larger primal heuristics and in-search greedy checks immediately destabilized the sweep (`N=783` flipped from `W` to `D` at about `0.86-1.01s`), while the residual cover bound was already behind on the `N<=814` anchor probe (`N=805 time=11.131s` vs `10.185s`, with `N=812` also slower). The simplest reason this round failed is that the hard cases are dominated by finding the first floor-cost incumbent, not by cheaply polishing it afterward, so extra same-`N` search or per-node side work mostly adds overhead and perturbs which cover gets exported; future attempts should avoid bounded plateau harvesting, larger warm-start neighborhoods, or extra residual lower-bound work unless they directly predict the first floor incumbent or stabilize export without changing the early `W/D` path.
 * 11. Tried four follow-ups aimed at warm-start export quality rather than the dual kernel itself: a broad same-heavy warm closure, a sound singleton-only witness extension, an exact residual-pair witness builder over the fixed post-solve heavy set, and a root-only balanced seed that packed extra heavy lines before DFS. The broad same-heavy closure was unsound and flipped `N=784` from the baseline `107` lines down to `106`, while the sound variants all preserved counts but regressed the full sweep (`singleton` to `total time=142.874s`, the exact residual witness builder to `149.875s`, and the balanced seed to `142.971s` against the validated baselines `149.779s` and `140.679s`).
 *    The useful structural finding is that the expensive tail splits cleanly in two: on the current baseline path `N=808`, `N=814`, and `N=821` re-found the same heavy-line set as the incoming warm start, so those costs are pure incumbent-proof / residual-completion misses, while `N=812`, `N=813`, `N=818`, and `N=825` needed genuinely different mostly old heavy sets (for example `N=813` kept only `83/99` warm heavy lines, added `19`, dropped `16`, and still chose zero newly activated lines). The simplest reason the round failed is that witness polishing only helps when the right heavy set is already present, and lightweight seed reshaping still does not steer the search onto the rare alternate `113/114` plateau cover early enough to offset its own overhead; future attempts should avoid more post-solve residual pairing searches or seed-only heavy-packing heuristics unless they can specifically flip `N=812/813/818` without paying another global same-`N` search.
 */
...
namespace config {
    ...
    constexpr int    kStartN               = 780; // Starting N for the sweep; 0 to start from 1
    constexpr int    kExecutionLimit       = 825; // Hard cap and default max N
    
    // Wall-clock time limit per N in seconds. Set to 0.0 for no limit.
...
std::vector<ExactGainSolver::Task> ExactGainSolver::build_frontier(Task root, WorkerContext& ctx) {
    ...
    // The 113-line tail is dominated by a few very heavy frontier tasks.
    // Over-splitting earlier hurts, but a deeper frontier here materially
    // improves load balance across the available cores.
    const unsigned frontier_multiplier = current_best_cost() >= 113 ? 128U
                                       : current_best_cost() >= 93 ? 16U
                                                                  : 4U;
...
int main(int argc, char** argv) {
    ...
    for (int n = std::max(kStartN, 1); n <= requested_n; ++n) {
        ...
        std::println("N={} prime={} lines={} time={} mode={} active={} nodes={} frontier={}"
