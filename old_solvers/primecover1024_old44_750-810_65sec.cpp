/*
 * Experiment log
 * 1. Tried a compile-time SIMD abstraction for BitMask1024::intersection_count and BitMask1024::and_not on an Intel(R) Core(TM) i9-9900K CPU @ 3.60GHz, exercising Tier 2 (__AVX2__).
 *    Objdump showed the baseline already emitted AVX2 for the inlined and_not path (vpandn/vmovdqu), while intersection_count remained scalar and + popcnt; the manual AVX2 paths changed that to vpand + vpextrq + popcnt, with unroll and dual-accumulator follow-ups also tested.
 *    All variants were bit-identical on every lines= output, but the final authoritative 810 sweeps averaged 92.065s wall versus the 91.795s baseline, so there was no reproducible win to keep; AVX-512 throttling was not a factor on this machine, and future attempts should avoid manual SIMD here unless repeated full-run measurements beat the solver's noise floor.
 * 2. Profiled the hardest tail case in a sandbox and found the real hotspot was the Lagrangian bound engine, not the 1024-bit mask primitives: at N=808 it made 877,056 lagrangian_upper_bound calls and spent about 112.7s inside lag(), including about 56.7s in eval_dense_fast, 16.6s building dense rows, and 14.2s in coordinate polish.
 *    Tried four distinct fixes aimed at that bottleneck: a more aggressive short warm-start with a tighter polish window, the tighter polish window alone, a DFS-time cover-inequality node bound, and deeper strong branching with kStrongBranchMaxDepth raised from 6 to 10; all were bit-identical on lines= output and therefore safe to compare directly.
 *    The first three either weakened pruning badly or added too much per-node overhead (for example N=800 rose to 15.357s under the short warm-start variant, and the node-bound variant nearly doubled several early D cases), while deeper strong branching only reduced nodes without a reproducible wall-time win: the two 810 sweeps came in at 100.038s total / 92.52s wall and 99.923s total / 94.87s wall versus the 99.658-100.266s total and 92.00-94.09s wall baseline band.
 *    Future attempts should avoid extra work on every DFS node or relaxing the current warm-start and polish settings unless the change demonstrably cuts the N=808 subgradient iteration load itself, because that gap-1 tail behavior is where the sweep is actually dominated.
 */
...
namespace config {
    ...
    constexpr int    kExecutionLimit       = 810; // Hard cap and default max N
    
    // Wall-clock time limit per N in seconds. Set to 0.0 for no limit.
    constexpr double kPerNTimeLimitSeconds = 200;
} // namespace config
...
private:
    ...
    static constexpr int kUniqueBranchActiveThreshold = 512;
    static constexpr int kUniqueBranchMinPoints       = 4;

    struct BranchChoice {
        int         line       = -1;
        long double exclude_ub = 0.0L;
        bool        via_unique = false;
    };
    ...
    [[nodiscard]] int choose_unique_cover_branch(Task& task, WorkerContext& ctx) const {
        if (task.active_count > kUniqueBranchActiveThreshold) return -1;
        ...
        int best_line   = -1;
        int best_unique = kUniqueBranchMinPoints - 1;
        int best_gain   = -1;
        ...
        for (int id : touched) {
            ...
            if (unique_points < kUniqueBranchMinPoints) continue;

            const int gain = positive_gain(task, id);
            if (gain <= 0) continue;

            const int checked_unique = exact_unique_points_for_line_with_threshold(task, id, kUniqueBranchMinPoints);
            if (checked_unique < kUniqueBranchMinPoints) continue;
    ...
    [[nodiscard]] BranchChoice choose_branch(Task& task, WorkerContext& ctx, long double node_gain_ub) const {
        if (const int unique_line = choose_unique_cover_branch(task, ctx); unique_line >= 0) {
            return {unique_line, exclude_branch_upper_bound(task, ctx, unique_line, node_gain_ub), true};
        }
        ...
        BranchChoice best{ shortlist[0].id, shortlist[0].exclude_ub, false };
    ...
    void dfs(Task& task, WorkerContext& ctx, int depth = 0) {
        ...
        // A line with >= 4 uniquely-covered active points is proved mandatory,
        // so skip the exclude branch and recurse on the forced include only.
        if (branch.via_unique) {
            ++ctx.stats.forced;
    ...
    [[nodiscard]] std::vector<Task> build_frontier(Task root, WorkerContext& ctx) {
        ...
        const unsigned frontier_multiplier = current_best_cost() >= 93 ? 16U : 4U;
