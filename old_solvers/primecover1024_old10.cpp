...
constexpr int    kStartN               = 0;   // N to start from in the timed sweep; set to 0 to run from 1
...
private:
    ...
    [[nodiscard]] LagrangianStats lagrangian_upper_bound_stats(
        const Task& task, int max_iters) const
    {
        ...
        std::vector<long double> best_y = y;
        ...
        if (kCoordinatePolishSweeps > 0) {
            ...
            productive_ids.reserve(lines_.size());
            std::vector<long double> line_slack(lines_.size(), 0.0L);

            for (int id = 0; id < static_cast<int>(lines_.size()); ++id) {
                if (!task.available[static_cast<std::size_t>(id)]) continue;
                const int rem = static_cast<int>(task.line_cover[static_cast<std::size_t>(id)]);
                if (rem < 3) continue;
                productive_ids.push_back(id);
    ...
    [[nodiscard]] long double lagrangian_upper_bound(const Task& task) const {
        return lagrangian_upper_bound_stats(task, 36).best_total;
    }
    ...
    bool& timed_out, int& out_lb, int& out_lb_raw, long long& out_nodes)
{
    ...
    const GainSolveResult result =
        solver.solve_for_n(
            n, active_ids, warm_start_solution, greedy_seed_line_ids, root_lb_floor);
...
int main(int argc, char** argv) {
    ...
    std::cout << "total time=" << format_seconds(total_elapsed) << '\n';
    std::cout << '\n'
              << "Output fields:\n"
