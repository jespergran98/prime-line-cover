...
private:
    ...
    // any point of U). The rule applies when |U| ≥ 4.
    //
    ...
    //   Because |U| ≥ 4 forces k ≥ ⌈|U|/2⌉ ≥ 2, we have Δ ≤ 0.  Hence there
    ...
    // loss of optimality.  The threshold is 4 (not 3).  For |U| = 3 we have
    // k ≥ ⌈3/2⌉ = 2; for k = 2, Δ = 0, and for k = 3, Δ = 0, so forcing
    // would also be safe, but the code maintains the original threshold
    // which has been empirically verified effective and correct.
    //
    ...
    void apply_dfs_forced_line_reduction(Task& task, WorkerContext& ctx) {
        // 1. Rebuild productive_degree / sole_productive_line from scratch.
        //    This eliminates any staleness introduced by parent branching.
        ...
        // 2. Forcing loop – identical logic to apply_root_forced_line_reduction
        //    except select_line (undo-aware) is used instead of
        //    select_line_without_undo, so every change is logged in ctx and
        //    will be undone when the node backtracks.
        ...
        while (true) {
            ...
            task.active.for_each_set_bit(words_, [&](int p) {
                const int ul = task.sole_productive_line[as_size(p)];
                if (ul >= 0) {
                    if (unique_count[as_size(ul)]++ == 0) touched.push_back(ul);
                    dirty_unique_count[as_size(ul)] +=
                        as_int(task.dirty_points[as_size(p)] != 0);
                }
            });
            ...
            for (int id : touched) {
                ...
                if (gain <= 0) continue;
                int checked = unique_points;
                if (dirty_unique_points > 0 && unique_points - dirty_unique_points < 4) {
                    checked = exact_unique_points_for_line_with_threshold(task, id, 4);
                    if (checked < 4) continue;
                }
                const bool better =
                    (checked > best_unique) ||
                    (checked == best_unique && gain > best_gain) ||
                    (checked == best_unique && gain == best_gain &&
            ...
            ++ctx.stats.forced;
            // Undo-aware: changes are logged in ctx and reverted on backtrack.
            select_line(task, ctx, best_line);
            task.current_gain += best_gain;
            task.current_choice.push_back(best_line);
            // After select_line, productive_ids has changed; rebuild metadata
            // so the next iteration's sole_productive_line scan is accurate.
            std::fill(task.productive_degree.begin(),
                      task.productive_degree.begin() + cur_n_, 0);
            std::fill(task.sole_productive_line.begin(),
                      task.sole_productive_line.begin() + cur_n_, kNoProductiveLine);
