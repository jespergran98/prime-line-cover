...
namespace config {
    ...
    constexpr double kPerNTimeLimitSeconds = 25.0;
} // namespace config
...
private:
    ...
    struct LagrangianScratch {
        ...
        void ensure(std::size_t line_count, int current_n) {
            ...
            if (subgrad.size() != n) subgrad.resize(n);
    ...
    void add_blocked_productive_impl(Task& task, int id) const {
        ...
        if (pos >= 0) return;
        pos = as_int(task.blocked_productive_ids.size());
        task.blocked_productive_ids.push_back(id);
    }

    void remove_blocked_productive_impl(Task& task, int id, std::vector<int>* removed_log) const {
        int& pos = task.blocked_productive_pos[as_size(id)];
        if (pos < 0) return;
        const int last_id = task.blocked_productive_ids.back();
        task.blocked_productive_ids[as_size(pos)] = last_id;
        task.blocked_productive_pos[as_size(last_id)] = pos;
        task.blocked_productive_ids.pop_back();
        pos = -1;
        if (removed_log) removed_log->push_back(id);
    }
    ...
    [[nodiscard]] int greedy_completion(const Task& task, WorkerContext& ctx, bool keep_lines) const {
        ...
        heap.clear();
        for (int id : task.productive_ids) heap.push_back({as_int(task.line_cover[as_size(id)]) - 2, id});
        ...
        const std::vector<std::size_t>& compact_offsets, const std::vector<int>& compact_points, bool keep_lines) const
    {
        ...
        for (std::size_t li = 0; li < compact_line_ids.size(); ++li) {
            heap.push_back({as_int(task.line_cover[as_size(compact_line_ids[li])]) - 2, as_int(li)});
        }
        std::ranges::make_heap(heap);
        ...
        while (!heap.empty()) {
            std::ranges::pop_heap(heap);
            const std::size_t li = as_size(heap.back().second);
            heap.pop_back();
    ...
    void build_compact_productive_lines(const Task& task, LagrangianScratch& scratch) const {
        ...
        active_points.clear();
        compact_line_ids.clear(); compact_offsets.clear(); compact_points.clear();
        ...
        for (std::size_t li = 0; li < compact_line_ids.size(); ++li) {
            for (std::size_t pos = compact_offsets[li]; pos < compact_offsets[li + 1]; ++pos) {
                ++ap_offsets[as_size(pt_to_ap[as_size(compact_points[pos])] + 1)];
            }
        }
    ...
    [[nodiscard]] long double lagrangian_upper_bound(const Task& task, WorkerContext& ctx, int max_iters, int* greedy_gain_out) const {
        ...
        if (scratch.dual_ready) {
            for (int p : active_points) y[as_size(p)] = std::clamp<DualValue>(best_y[as_size(p)], 0.0, 1.0);
        } else {
            for (int ai = 0; ai < as_int(active_points.size()); ++ai) {
                ...
                y[as_size(p)] = std::min<DualValue>(1.0, best_density);
            }
        }
        ...
        auto eval_dense_fast = [&](const std::vector<DualValue>& yy, std::vector<DualValue>* subgradient) -> DualValue {
            ...
            for (int p : active_points) {
                value += yy[as_size(p)];
                if (subgradient) (*subgradient)[as_size(p)] = 1.0;
            }
            for (std::size_t li = 0; li < compact_line_ids.size(); ++li) {
                DualValue sum_y = 0.0;
                for (std::size_t pos = compact_offsets[li]; pos < compact_offsets[li + 1]; ++pos) sum_y += yy[as_size(compact_points[pos])];
                const DualValue rc = static_cast<DualValue>(compact_rem2[li]) - sum_y;
                if (rc > 1e-18) {
                    value += rc;
                    if (subgradient) {
                        for (std::size_t pos = compact_offsets[li]; pos < compact_offsets[li + 1]; ++pos)
                            (*subgradient)[as_size(compact_points[pos])] -= 1.0;
                    }
                }
            }
            return value;
        ...
        auto eval_dense_exact = [&](const std::vector<DualValue>& yy, bool store_slack) -> long double {
            ...
            for (int p : active_points) value += static_cast<long double>(yy[as_size(p)]);
            for (std::size_t li = 0; li < compact_line_ids.size(); ++li) {
                const int id = compact_line_ids[li];
                long double sum_y = 0.0L;
                for (std::size_t pos = compact_offsets[li]; pos < compact_offsets[li + 1]; ++pos)
                    sum_y += static_cast<long double>(yy[as_size(compact_points[pos])]);
        ...
        const long double best_exact = eval_dense_exact(y, false);
        DualValue best = static_cast<DualValue>(best_exact);
        for (int p : active_points) best_y[as_size(p)] = y[as_size(p)];

        const long double initial_total = static_cast<long double>(task.current_gain) + best_exact;
        scratch.dual_ready = true;
        if (gain_upper_bound_cannot_beat_best_cost(initial_total)) return initial_total;
        ...
        auto maybe_confirm_prune = [&](DualValue candidate_best) -> bool {
            ...
            const long double exact_total = static_cast<long double>(task.current_gain) + eval_dense_exact(best_y, false);
        ...
        for (int iter = 0; iter < max_iters; ++iter) {
            ...
            DualValue norm2 = 0.0;
            for (int p : active_points) {
                const DualValue g = subgrad[as_size(p)];
                norm2 += g * g;
            }
            ...
            for (int p : active_points) {
                DualValue next = y[as_size(p)] - step * subgrad[as_size(p)];
        ...
        if (kCoordinatePolishMaxSweeps > 0) {
            ...
            for (std::size_t li = 0; li < compact_line_ids.size(); ++li) {
                ...
                for (std::size_t pos = compact_offsets[li]; pos < compact_offsets[li + 1]; ++pos)
                    slack -= best_y[as_size(compact_points[pos])];
                wls_c[li] = slack;
            }

            auto slack_objective = [&]() -> DualValue {
                DualValue value = 0.0;
                for (int p : active_points) value += best_y[as_size(p)];
                for (std::size_t li = 0; li < compact_line_ids.size(); ++li) {
            ...
            for (int sweep = 0; sweep < kCoordinatePolishMaxSweeps; ++sweep) {
                ...
                for (int ai = 0; ai < as_int(active_points.size()); ++ai) {
                    const int p = active_points[as_size(ai)];
                    const DualValue old = best_y[as_size(p)];
        ...
        return static_cast<long double>(task.current_gain) + eval_dense_exact(best_y, true);
    }
    ...
    void collect_blocked_productive_lines(const Task& task, std::vector<int>& out) const {
        out = task.blocked_productive_ids;
        if (out.size() > 1) std::ranges::sort(out);
    }
