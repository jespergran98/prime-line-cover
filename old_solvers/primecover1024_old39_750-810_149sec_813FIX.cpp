...
namespace config {
    ...
    constexpr int    kExecutionLimit       = 815;   // hard cap and default run-to N
    // Maximum wall-clock seconds allowed per N. Set to 0.0 to disable (no limit). 60=1 min
    constexpr double kPerNTimeLimitSeconds = 250;
} // namespace config
...
private:
    ...
    void undo_to(Task& task, WorkerContext& ctx, UndoMark mark) const {
        ...
        // Restore point metadata FIRST (stale, missing the line that was disabled)
        ...
        // THEN re‑insert productive lines that were removed; they will increment the
        // just‑restored degree and sole line, correcting the stale metadata.
