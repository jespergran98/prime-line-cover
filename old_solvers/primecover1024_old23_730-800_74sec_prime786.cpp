 ...
 * 
 * Experiment Log:
 *   1. Tried correcting and then selectively exact-checking the productive
 *      point metadata used by forced-line reduction so the `N=786` overcount
 *      would drop from `108` to the oracle `107` without materially changing
 *      the tail cost. Verified runs did fix `N=786`, but every correctness-
 *      preserving variant slowed the hard tail too much (`N=786..800`
 *      increased from `49.939s` baseline to `73.059s` for the full metadata
 *      fix, with `N=795` rising from `13.397s` to `21.375s` and `N=800` from
 *      `19.709s` to `30.550s`). The simplest explanation is that the current
 *      tail speed depends on the stale forced-line metadata driving much more
 *      aggressive pruning, so future attempts should avoid globally tightening
 *      or exact-checking that path unless they also add a comparably cheap
 *      compensating prune.
 *   2. Tried repairing the stale forced-line metadata path so `N=786` would
 *      drop from `108` to the oracle `107` without materially slowing the
 *      tail. All three correctness-preserving variants did fix `N=786`, but
 *      each made the hard tail substantially slower (`N=795` rose from
 *      `15.046s` baseline to `21.323s`, `23.076s`, and `22.970s`, while
 *      `N=800` rose from `18.412s` to `30.961s`, `32.746s`, and `32.503s`).
 *      The simplest explanation is that even selective repairs to the covered-
 *      point productive metadata disturb the aggressive pruning behavior that
 *      keeps the tail fast, so future attempts should avoid changing that undo
 *      / forced-line path unless they also preserve the original pruning
 *      pattern or add a compensating cheap prune.
 *   3. Tried repairing the undoable covered-point save so `N=786` would drop
 *      from `108` to `107` with minimal or no performance impact. The fix and
 *      two more selective variants all corrected `N=786`, but the hard tail
 *      still slowed too much (`N=795` rose from `12.949s` baseline to
 *      `22.622s`, `22.321s`, and `19.518s`, while `N=800` rose from `18.214s`
 *      to `32.354s`, `31.605s`, and `26.789s`). The simplest explanation is
 *      that the stale forced-line metadata is currently providing a large share
 *      of the tail pruning, so future attempts should avoid correctness fixes
 *      that only remove that pruning unless they also add a comparably cheap
 *      replacement prune.
 *   4. Tried preserving selected-line point metadata in both the undoable and
 *      non-undo frontier selection paths, and separately relaxing the exclude-
 *      branch prune, so `N=786` would drop from `108` to `107` with minimal or
 *      no performance impact. Verified runs were inconsistent: the direct-start
 *      `N=786` debug reproduction did reach `107`, but the real `730..800`
 *      carry-over run either still returned `N=786 = 108` after the selection-
 *      order fixes or became much slower when the exclude prune was relaxed
 *      (`N=794` rose to `19.870s`, `N=796` to `24.483s`, with the interrupted
 *      run already at `107s` total). The simplest explanation is that the bad
 *      `N=786` result depends on a full-sweep incumbent/frontier interaction
 *      beyond the local covered-point metadata path, so future attempts should
 *      avoid treating an isolated `N=786` reproduction as sufficient and
 *      should verify every prune change against the full `730..800` run before
 *      keeping it.
 *   5. Tried three narrower repairs to fix the `N=786` overcount from `108`
 *      to `107` with minimal or no tail slowdown: exact-checking every forced-
 *      line candidate, exact-checking only the chosen forced line, and
 *      preserving pre-select covered-point metadata only on the undoable
 *      include path. All three variants did correct `N=786`, but the full
 *      `730..800` sweep slowed far too much (`N=789` rose from `7.934s` /
 *      `7.984s` baseline to `12.849s`, `12.903s`, and `13.101s`, while the
 *      latter two runs reached `102.261s` total and `N=795 = 21.331s` before
 *      the final abort). The simplest explanation is that the current tail
 *      speed depends heavily on the stale forced-line behavior pruning hard
 *      subtrees, so future attempts should avoid fixes that only disable or
 *      bypass those false forces unless they also add a comparably cheap
 *      replacement prune or a proof-driven repair limited to the exact bad
 *      transition.
 *   6. Tried three narrower repairs aimed at fixing the `N=786` line-count
 *      miss from `108` to `107` with minimal or no tail slowdown: an undo-
 *      only re-add of selected-line contributions, a dirty-state exact-check
 *      of forced lines, and a depth-gated dirty-state exact-check. All three
 *      variants did correct `N=786`, but each still slowed the hard tail too
 *      much (`N=795` rose from `13.000s` / `13.094s` baseline to `21.614s`,
 *      `22.735s`, and `22.929s`, while `N=800` rose from `18.516s` /
 *      `18.671s` baseline to `30.452s`, `32.104s`, and `32.309s`). The
 *      simplest explanation is that even late or dirty-state repairs suppress
 *      the false forces that currently provide a large share of the tail
 *      pruning once the search is deep, so future attempts should avoid broad
 *      deep-tree repairs to forced-line behavior unless they also recover
 *      equivalent pruning or isolate the single bad `N=786` transition more
 *      precisely.
 *   7. Isolated the current `N=786` miss to a rollback state bug in the
 *      forced-line metadata: after undo, stale `productive_degree` /
 *      `sole_productive_line` values can survive long enough to trigger a
 *      false kernel reduction. The direct witness was
 *      `FALSE_FORCE depth=51 chosen=51 best_line=1149 meta_unique=5 meta_gain=4
 *      exact_line=-1 exact_unique=0 exact_gain=0`, which changes the final
 *      96 heavy lines from leaving `22` uncovered points (optimal, `107`
 *      total lines) to leaving `23` uncovered points (reported `108`). Tried
 *      four minimal repairs: restoring covered-point metadata before
 *      reinsert, a late full exact-check of forced lines, a candidate-only
 *      exact-check, and a depth-gated undo repair. All four corrected
 *      `N=786`, but none met the performance requirement on the full
 *      `730..800` sweep: the best variant still rose from `76.238s..76.582s`
 *      baseline total time to `81.895s` (`N=795 13.347s`, `N=800 19.404s`),
 *      while the others landed at `98.702s`, `102.951s`, and `108.305s`.
 *      The most likely reason is that the stale forced-line metadata is
 *      currently supplying a meaningful share of deep-tail pruning, so future
 *      attempts should target the single bad transition more narrowly or add
 *      a comparably cheap replacement prune before removing that behavior.
 *   8. Reconstructed the live `N=786` miss directly on the real `730..800`
 *      carry-over sweep. The failing run still chose `96` heavy lines, but
 *      they covered only `763` points and left `23` residual points (`108`
 *      total lines), while an exact-forced debug run kept `96` heavy lines,
 *      covered `764` points, and left `22` residual points (`107` total).
 *      The first permanent coverage deficit appears at selection depth `84`
 *      (`727` covered points versus `728` on the corrected path), but the
 *      causal branch happens earlier: `FALSE_FORCE record_undo=1 depth=51
 *      chosen=51 best_line=1149 meta_unique=5 meta_gain=4 exact_line=-1
 *      exact_unique=0 exact_gain=4`, which again pins the bug on undo-restored
 *      forced-line metadata. Tried three direct repairs in this pass:
 *      preserving all selected-point metadata before disabling the chosen line
 *      (fixed `N=786`, but slowed the full tail too much: `N=795 22.222s`,
 *      `N=800 31.656s`, `106.417s` total), preserving only degree-`2` selected
 *      points (an isolated `N=786` run dropped to `107`, but the real
 *      carry-over sweep still reported `108`), and preserving degree-`1/2`
 *      selected points (still reported `108`). The most likely reason is that
 *      the full repair removes stale false-force pruning that currently keeps
 *      the hard tail fast, while the narrower filters do not repair the exact
 *      carry-over transition. Future attempts should keep the `depth=51`
 *      false-force witness and the `depth=84` permanent-coverage divergence as
 *      regression checks, and should look for a cheap validation targeted only
 *      at undo-restored degree-`2` transitions before forcing a line.
 *   9. Re-ran the authoritative baseline twice sequentially (`N=786 = 108`)
 *      and traced the real carry-over failure again. The direct witness stayed
 *      `FALSE_FORCE depth=51 chosen=51 best_line=1149 meta_unique=5 meta_gain=4
 *      exact_line=-1 exact_unique=0 exact_gain=4`, and the exact-forced
 *      comparison showed the same `96` heavy lines but a different residue:
 *      the failing path covered `763` points and left `23` residual points
 *      (`12` residual lines, `108` total), while the corrected path covered
 *      `764` points and left `22` residual points (`11` residual lines, `107`
 *      total). The first permanent coverage deficit still appeared at depth
 *      `84`. Tried three new dirty-state candidate-validation repairs aimed at
 *      fixing only the stale undo metadata: marking every restored point dirty,
 *      marking only restored stale-degree-`1` points dirty, and marking every
 *      restored point with positive saved degree dirty. The first and third
 *      variants did correct `N=786` to `107`, but the full `800` sweep slowed
 *      from the `74.706s..76.616s` baseline totals to `104.423s` and `98.085s`
 *      (`N=795` rose to `21.072s` / `19.064s`, `N=800` to `30.313s` /
 *      `27.898s`); the middle variant was a bit cheaper (`94.663s` total) but
 *      still reported `N=786 = 108` on the real carry-over run. The most
 *      likely reason is that any repair broad enough to suppress the false
 *      forced lines also removes a large share of the deep-tail pruning, while
 *      narrower dirty filters miss points restored above stale degree `1` that
 *      only become false-unique after later productive-line removals. Future
 *      attempts should keep the `depth=51` false-force witness and the
 *      `depth=84` permanent-divergence check, and should look for a cheaper
 *      repair around restored positive-degree points that also recovers an
 *      equivalent deep-tail prune before replacing the current behavior.
 *   10. Reconstructed the live carry-over miss again with fresh trace files
 *       from the authoritative baseline and an exact-forced comparison run.
 *       The failing `N=786` solution still chose `96` heavy lines but covered
 *       only `763` points, leaving `23` residual points (`12` residual lines,
 *       `108` total), while the exact-forced comparison kept `96` heavy lines,
 *       covered `764` points, and left `22` residual points (`11` residual
 *       lines, `107` total). The first permanent coverage deficit again
 *       appeared at selection depth `84` (`727` covered points versus `728` on
 *       the corrected path). The new direct witness on the final failing path
 *       was a shallow false force from stale rollback metadata:
 *       `FALSE_FORCE record_undo=1 depth=25 chosen=25 best_line=33
 *       meta_unique=5 meta_gain=6 best_line_exact_unique=0 exact_line=-1
 *       exact_unique=0 exact_gain=0`, with follow-up shallow witnesses at
 *       depths `29`, `30`, and `31`. Tried five narrower candidate-validation
 *       repairs aimed at containing only those false forces: validating all
 *       undo-path forced candidates with `meta_unique <= 6`, tightening that
 *       gate to `<= 5`, tightening again to `== 5`, and then adding shallow
 *       depth gates (`depth <= 31`) for the `<= 5` and `<= 6` variants. The
 *       `<= 6` and `<= 5` variants did fix `N=786` to `107`, but the full
 *       `800` sweep still slowed too much (`N=795` rose to `21.871s` and
 *       `N=800` was still at `31.0s` when aborted for the `<= 6` gate), while
 *       the three tighter variants all reverted to `N=786 = 108`. The most
 *       likely reason is that suppressing even these shallow false forces
 *       removes a meaningful share of the tail pruning, while narrower gates
 *       miss at least one of the exact bad transitions. Future attempts should
 *       keep the `depth=25/29/30/31` shallow false-force witnesses and the
 *       `depth=84` permanent-divergence point as regression checks, and should
 *       focus on repairing the rollback transition that creates stale
 *       `productive_degree` / `sole_productive_line` values rather than on
 *       post-hoc forced-line validation.
 *   11. Replayed the current authoritative baseline twice (`75.195s` /
 *       `75.393s`, `N=786 = 108`) and then retried the mandated rollback
 *       direction on a synced working copy, paired with three cheap current-
 *       state replacement prunes. The rollback reorder itself — restoring
 *       covered-point productive metadata before reinsert on the undoable
 *       path — still fixed `N=786` to `107`, but on the current source state
 *       it slowed the same instrumented build from `78.622s` baseline
 *       (`N=795 14.050s`, `N=800 19.718s`) to `108.497s` (`N=795 22.866s`,
 *       `N=800 32.311s`, `616205` nodes). Tried three prune variations:
 *       lowering the valid forced-line threshold from `>= 4` to `>= 3`
 *       everywhere (`108.005s`, `N=795 22.680s`, `N=800 32.759s`,
 *       `619670` nodes), subtracting `best_y[p]` from
 *       `exclude_branch_upper_bound()` when the branch line owns a truly
 *       unique productive point (`110.112s`, `N=795 23.075s`,
 *       `N=800 32.654s`, `615421` nodes), and combining both
 *       (`109.894s`, `N=795 22.923s`, `N=800 32.924s`, `619542` nodes).
 *       Branched-node tracing on `N=795` / `N=800` at depths `>= 60` showed
 *       why they failed: the repaired run still visited `3128` corrected-only
 *       branching states on `N=795` and `3323` on `N=800`; their `max_unique`
 *       split was `N=795: {0:60, 1:1862, 2:482, 3:724}` and
 *       `N=800: {0:4, 1:528, 2:1446, 3:1345}`, so the broad `>= 3` force only
 *       touched a minority of the missed tail, while the unique-point dual
 *       penalty recovered effectively none of it. The traced stale baseline's
 *       branching states were dominated by impossible rollback metadata
 *       (`deg0 ~= 31..37`, `deg1 ~= 19..33`), whereas the corrected-only
 *       states stayed at valid low-degree scarcity (`deg0 ~= 4`,
 *       `deg1 ~= 3..6`), so any replacement limited to degree-1 / true-unique
 *       logic is too narrow. Future attempts should first reconstruct the
 *       exact attempt-7 branch that produced the logged `81.895s` behavior on
 *       this codebase, then target the `ub == need` corrected-only branch
 *       states with a cheap bound that also handles the large
 *       `max_unique = 1/2` population instead of only the `max_unique = 3`
 *       slice.
 */
