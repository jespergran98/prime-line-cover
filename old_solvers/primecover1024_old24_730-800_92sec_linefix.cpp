    ...
    const std::vector<int>& chosen_line_ids)
{
    ...
    for (std::size_t depth = 0; depth < chosen_line_ids.size(); ++depth) {
        ...
        const auto& line = lines[static_cast<std::size_t>(id)];
    ...
    replay.residual_lines =
        ceil_half(static_cast<int>(replay.residual_points.size()));
    return replay;
}
...
private:
        ...
        int n, const std::vector<int>& active_ids) const
    {
        ...
        root.dirty_points.assign(static_cast<std::size_t>(n), 0);
    ...
    void prepare_worker_context(WorkerContext& ctx) const {
        ...
        if (ctx.covered_point_dirty.capacity() < static_cast<std::size_t>(cur_n_))
            ctx.covered_point_dirty.reserve(static_cast<std::size_t>(cur_n_));
    ...
    {
        int count = 0;
        for (int p : lines_[static_cast<std::size_t>(target_line)].points) {
            if (p >= cur_n_) break;
            if (!task.active.test(p)) continue;
        ...
        const Task& task, int target_line, int threshold) const
    {
        ...
        for (int p : lines_[static_cast<std::size_t>(target_line)].points) {
            ...
            if (!task.dirty_points[static_cast<std::size_t>(p)]) {
                if (task.productive_degree[static_cast<std::size_t>(p)] == 1 &&
                    task.sole_productive_line[static_cast<std::size_t>(p)] == target_line)
                    ++clean_unique;
            } else {
                ++dirty_active_points;
            }
        ...
        for (int p : lines_[static_cast<std::size_t>(target_line)].points) {
            if (p >= cur_n_) break;
            if (!task.active.test(p)) continue;
            if (!task.dirty_points[static_cast<std::size_t>(p)]) continue;
        ...
        const Task& task, WorkerContext& ctx) const
    {
        ...
        task.active.for_each_set_bit(words_, [&](int p) {
            ...
            if (exact_degree == 1) {
                int& count = exact_unique_count[static_cast<std::size_t>(exact_sole)];
                if (count++ == 0)
        ...
        for (int id : exact_touched) {
            const int unique_points =
                exact_unique_count[static_cast<std::size_t>(id)];
            exact_unique_count[static_cast<std::size_t>(id)] = 0;
    ...
    void undo_to(Task& task, WorkerContext& ctx, UndoMark mark) const {
        ...
        for (std::size_t i = mark.covered_point_metadata;
            ...
            // Any restored point that still has another productive line is
            // missing exactly one contribution from the chosen line after undo.
            // It can therefore become a false unique later if enough of those
            // other productive lines drop out.
            task.dirty_points[static_cast<std::size_t>(p)] =
                static_cast<unsigned char>(ctx.covered_point_degree[i] > 0);
        }
        ...
        while (true) {
            ...
            task.active.for_each_set_bit(words_, [&](int p) {
                ...
                if (unique_line >= 0) {
                    ...
                    dirty_unique_count[static_cast<std::size_t>(unique_line)] +=
                        static_cast<int>(task.dirty_points[static_cast<std::size_t>(p)] != 0);
                }
            });
            ...
            for (int id : touched) {
                ...
                const int dirty_unique_points =
                    dirty_unique_count[static_cast<std::size_t>(id)];
                unique_count[static_cast<std::size_t>(id)] = 0;
                dirty_unique_count[static_cast<std::size_t>(id)] = 0;
                if (unique_points < 4) continue;
