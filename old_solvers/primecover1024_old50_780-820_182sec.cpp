...
private:
    ...
    //   In any solution omitting L, each point of U must be covered by a
    //   light line (covering at most 2 active points). A light line covers
    //   at most one point of U — if it covered two, it would be a heavy line
    //   for those points, contradicting exclusive dependency. Therefore U
    //   requires at least |U| distinct light lines. Replacing them with L
    //   (cost 1) and re-covering the displaced points (at most ceil(|U|/2)
    //   new lines) gives cost ≤ original – |U| + 1 + ceil(|U|/2)
    //                         = original – (|U| – 1 – ceil(|U|/2)).
    //   For |U| ≥ 4 the saving is ≥ 1, so any solution omitting L is
    //   strictly suboptimal.  QED.
    //
    // Consequently L is forced into the partial solution without any loss of
    // optimality. The threshold is 4 (not 3): at |U| = 3 the saving is 0
    // (break-even), giving no strict improvement; |U| ≥ 4 is unconditional.
    //
    // BUG FIX (N=813 regression): Never apply this rule inside dfs().
    // It relies on sole_productive_line metadata that becomes stale after
    // branching, causing false positives (forcing a suboptimal line). N=813
    // returned 114 instead of 113, with the error varying by kExecutionLimit
    // (the active line set changes with the search horizon). Apply only at
    // root or frontier expansion, where metadata is guaranteed fresh.
    // ----------------------------------------------------------------------
