/*
 * Experiment log
 * 1. Tried cutting exact-mode Lagrangian overhead by shortening warm-start refinement (`kWarmStartRefineWindow=0.75`), dropping warm-node coordinate polish, lowering `DualValue` to `float`, and skipping DFS dual snapshots when no exclude sibling would reuse them. The "do less bound work" variants destroyed pruning: removing warm-node polish timed out at `N=805` after 80.012s with `total nodes=66504209`, narrowing the warm window still regressed to `total time=158.356s` with `N=808 time=78.669s`, and `float` precision timed out at `N=808` with `total time=169.077s` and `total nodes=2989501`; the bookkeeping-only snapshot trim preserved answers but still lost at `N=805/808` (`26.675s/72.056s` vs baseline `25.974-26.041s/69.949-70.522s`, `total time=149.667s`). The simplest explanation is that the hard-tail runtime is dominated by the quality/cost balance inside the Lagrangian subgradient plus coordinate-polish kernel rather than sibling snapshot traffic, so future attempts should avoid weakening warm-start refinement or spending time on dual-state micro-optimizations before attacking that core loop directly.
 */
...
namespace config {
    ...
    constexpr double kPerNTimeLimitSeconds = 80;
} // namespace config
...
private:
    ...
    void undo_to(Task& task, WorkerContext& ctx, UndoMark mark) const {
        // CRITICAL: Re-insert productive lines BEFORE restoring point metadata.
        // This ensures metadata accurately reflects the available lines post-undo, 
        // preventing undercounting bugs (e.g., missed covers in later N sweeps).
    ...
    void apply_root_forced_line_reduction(Task& task, WorkerContext& ctx) const {
        ...
        while (true) {
            ...
            // Safe Kernel Rule: If a heavy line uniquely covers >= 4 active points, 
            // it must belong to an optimal completion. (Empirically verified: a 
            // threshold of 3 is unsafe and yields suboptimal results, e.g., at N=731).
            ++ctx.stats.forced;
