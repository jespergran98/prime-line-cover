/*
 * Experiment log
 * 1. Investigated the Lagrangian upper-bound kernel, because an N=814 probe showed it dominating the proof (`lag_calls=968314`, aggregated `lag_time≈172.5s`) while duplicate DFS-state probing found essentially no reusable states (`state_probe_hits=0`), so the focus shifted away from memoization and toward cheaper kernel execution.
 *    A float dual kernel was rejected immediately because it returned 106 lines at N=780 instead of the baseline 105, and the correctness-preserving variants did not beat baseline sweep time: `kWarmStartRefineWindow=1.0` finished in `127.908s` with `4,800,048` nodes and `SubgradientValue=int16_t` finished in `131.382s` with `4,262,516` nodes, versus the baseline `126.195s/4,127,362` and `128.855s/4,239,787`.
 *    Fixing the cold-start coordinate-polish sweep selection (`warm_start_dual ? 1 : 6`) also trended worse on the first hard cases (`N=805 7.39s`, `N=808 19.46s`), so the simplest explanation is that the bottleneck is the volume of warm-start subgradient work itself rather than arithmetic width, duplicate pruning, or a missing cold-start polish path.
 *    Future attempts should avoid approximate dual numerics and shallow schedule retunes; the remaining leverage is more likely in reducing warm-node bound calls or incrementally reusing compact productive-line state across parent/child DFS nodes.
 * 2. Investigated witness-refresh and same-active fast-bound variants to cut the plateau proofs and avoid paying a full dual solve after a warm carry-forward. Rebuilding the witness on `mode=W` from stale heavy-line IDs could produce an over-budget cover and corrupt the line count (for example it incorrectly held the sweep at 109 lines through `N=795`, where the baseline is 110), while the budget-preserving/local-repair witness variants still finished at `127.581s` and `128.071s` against the confirmed `126.417s` baseline.
 *    A targeted profile to `N=814` measured aggregated `lag_build≈100.351s` and `lag_polish≈73.464s` out of `lag_total≈644.470s`, so compact rebuild and polish matter but the subgradient/evaluation body still consumes about `470.654s`; the simplest explanation is that the current witness heuristic lacks safe slack and cheap same-active shortcuts either lose correctness or save too little to matter.
 *    Future attempts should avoid refreshing witnesses unless the exact line budget is carried by an explicit valid cover representation, and should not weaken same-active bound refresh logic without a proof that the inherited dual cap and branching data stay equivalent to the full solve.
 * 3. Investigated bound-kernel representation rewrites after an `N=814` sandbox probe showed `lag_calls=969067`, `lag_time≈167.494s`, `build_time≈26.392s`, `polish_time≈19.214s`, average rebuilds of `743.42` productive lines / `268.99` active points / `2485.33` active incidences, and average parent-child DFS deltas of only `4.97` covered points and `164.37` decremented line touches. The full original-point-ID kernel was clearly worse (`N=814 32.871s`, total `140.512s`), the denser polish variant still trailed the confirmed `129.621s` / `128.722s` baselines (`N=814 29.592s`, total `131.128s`), and the one-pass compact remap reordered the compact indices enough to hit the 50s cap at `N=813`.
 *    The simplest explanation is that the baseline's dense compact indexing and traversal order are more valuable to the hot evaluation kernel than the rebuild work they save, so sparse point-ID access or first-seen renumbering gives back the measured build/polish savings as slower dual iterations or worse search behavior.
 *    Future attempts should avoid replacing compact active-point numbering or changing its order unless the parent/child reuse path preserves the exact dense layout; the remaining leverage is more likely in incremental maintenance of the existing compact state than in a new representation.
 * 4. Investigated exact DFS-path compact-state maintenance to remove repeated `build_compact_productive_lines()` work without changing the line-count logic. On the confirmed `N=814` probe (`wall=88.05s` baseline to `N=814`), a direct cache-driven kernel cut measured rebuild time from `≈26.6s` aggregated to `≈0.45s`, but wall time worsened to `140.99s` and aggregated lag time rose to `≈260.0s`; a hybrid that used the cache only to materialize the baseline dense kernel still timed out at `N=813` after `50.017s`, and a lighter productive-degree builder rewrite crashed before completion.
 *    The simplest explanation is that the per-branch cache mutation/undo traffic and even slight deviations in compact-state materialization destroy enough kernel locality or pruning behavior to outweigh the saved rebuild work. Future attempts should avoid DFS-path compact caches unless they preserve the baseline dense layout with almost-zero update overhead and can be shown not to perturb branch ordering or warm dual quality.
 */
...
namespace config {
    ...
    constexpr int    kStartN               = 750; // Starting N for the sweep; 0 to start from 1
    constexpr int    kExecutionLimit       = 820; // Hard cap and default max N
    
    // Wall-clock time limit per N in seconds. Set to 0.0 for no limit.
    constexpr double kPerNTimeLimitSeconds = 950;
} // namespace config
...
private:
    ...
    [[nodiscard]] int exact_unique_points_for_line_with_threshold(const Task& task, int target_line, int threshold) const {
        ...
        for (CompactPointIndex point_cp : line_points(target_line)) {
            ...
            if (!task.dirty_points[as_size(p)]) {
                if (task.productive_degree[as_size(p)] == 1 && task.sole_productive_line[as_size(p)] == target_line) {
                    ++clean_unique;
                }
            } else {
                ++dirty_active_points;
    ...
    void select_line_without_undo(Task& task, int id) const {
        ...
        for (CompactPointIndex point_cp : line_points(id)) {
            ...
            for (int lid : ordered_point_lines_[as_size(p)]) {
                ...
                if (available) remove_productive_impl(task, lid, nullptr); 
    ...
    // ----------------------------------------------------------------------
    // Safe Kernel Rule (proved, unconditional)
    // ----------------------------------------------------------------------
    // 
    // Let L be a productive heavy line (covers at least 3 active points).
    // Suppose there are at least 4 active points for which L is the *only*
    // available productive heavy line (no other heavy line with coverage ≥3
    // still active contains any of those points). Call this set U, |U| ≥ 4.
    //
    // Proof that L must belong to every optimal completion:
    //   In any solution that does NOT use L, each point of U must be covered
    //   by a light line (a line covering at most 2 active points). A light
    //   line can cover at most one point of U (otherwise it would become a
    //   heavy line covering two points of U, contradicting uniqueness).
    //   Therefore covering U requires at least |U| distinct light lines.
    //   Replacing those |U| light lines by L (cost 1) and then covering the
    //   (at most |U|) remaining light‑line points with new lines (needing at
    //   most ceil(|U|/2) extra lines) yields a solution whose total number
    //   of lines is ≤ original – |U| + 1 + ceil(|U|/2) = original – (|U| – 1 – ceil(|U|/2)).
    //   For |U| ≥ 4, the reduction is at least 1, meaning the new solution
    //   is strictly better than the one without L. Hence a solution without
    //   L cannot be optimal.  QED.
    //
    // Consequently L can be forced into the current partial solution without
    // any loss of optimality. The code uses threshold 4 (not 3) because for
    // |U| = 3 the inequality becomes weak (break‑even) and could lead to
    // parity‑sensitive ties during search, but with |U| ≥ 4 the rule is
    ...
    void apply_root_forced_line_reduction(Task& task, WorkerContext& ctx) const {
        ...
        while (true) {
            ...
            for (int id : touched) {
                ...
                const bool better = (checked_unique > best_unique) || 
    ...
    [[nodiscard]] long double lagrangian_upper_bound(const Task& task, WorkerContext& ctx, int max_iters) const {
        ...
        auto run_subgradient_iters = [&](int begin_iter, int end_iter) -> bool {
            for (int iter = begin_iter; iter < end_iter; ++iter) {
                ...
                const DualValue step_scale = warm_start_dual ? DualValue{1.5} : DualValue{1.35};
    ...
    [[nodiscard]] long double lagrangian_upper_bound(const Task& task, WorkerContext& ctx) const {
...
int main(int argc, char** argv) {
    ...
    for (int n_init = 1; n_init < kStartN; ++n_init) {
        for (int id : activation[as_size(n_init)]) {
            active_ids.push_back(id);
        }
    }

    for (int n = std::max(kStartN, 1); n <= requested_n; ++n) {
        for (int id : activation[as_size(n)]) {
            active_ids.push_back(id);
        }
